# Instruksi AI Agent — White Box Testing Terbaru SIARSIP BPKPAD Balangan

Dokumen ini adalah runbook untuk AI agent yang akan menjalankan white box testing berdasarkan repository Android Kotlin `Sistem-Arsip-BPKPAD-Balangan-main4.zip` dan skenario uji terbaru setelah perubahan ERD. Perubahan utama pada baseline terbaru adalah masuknya struktur lokasi fisik berbasis `Room`, `Shelf`, dan `Box`, serta relasi ke `ArchiveDocument` melalui `idStorageLocation` dan metadata lokasi.

## 1. Tujuan Eksekusi

Agent harus memvalidasi jalur internal kode, bukan hanya perilaku tampilan. Fokus pengujian adalah:

1. State management dan validasi pada ViewModel.
2. Query Room DAO, mapper, dan repository local-first.
3. Alur lokasi fisik: room, shelf/rak, box, dan mapping ke dokumen.
4. Bulk upload dari staging menuju arsip final.
5. Relasi `transaction_bundles`.
6. Audit trail melalui activity log.
7. OCR, AI parsing, dan autofill metadata.
8. Dashboard, rekap, import/export Excel, dan utility formatting.

## 2. Struktur Project yang Harus Diperiksa

```bash
cd /mnt/data
rm -rf Sistem-Arsip-BPKPAD-Balangan-main
unzip -q Sistem-Arsip-BPKPAD-Balangan-main4.zip
cd Sistem-Arsip-BPKPAD-Balangan-main

find app/src/main/java/com/example/arsipbpkpad -maxdepth 5 -type f | sort
find app/src/test app/src/androidTest -type f | sort
```

Project valid jika berisi file berikut:
...
Buat `local.properties` minimal bila belum ada. Jangan memakai secret produksi.
...
Jika Gradle wrapper mencoba mengunduh distribusi dan jaringan tidak tersedia, tandai eksekusi sebagai `Blocked - Gradle distribution unavailable`, lalu tetap lanjutkan static review terhadap test file dan source code.

## 4. Pemeriksaan Pra-Kompilasi Wajib

Sebelum menjalankan semua test, agent wajib mengecek dua potensi ketidaksesuaian akibat perubahan kode:

1. `RapidInputViewModel` membutuhkan dependency `StorageLocationRepository`. Test lama harus menambahkan mock dependency tersebut saat membuat instance ViewModel.
2. Pada kode terbaru terlihat referensi `showDuplicateWarning` di `RapidInputViewModel`. Pastikan field tersebut ada di `RapidInputUiState`. Jika tidak ada, kompilasi akan gagal dan status test yang bergantung pada ViewModel adalah `Blocked - compile error`.

Command static check:

```bash
grep -R "class RapidInputViewModel" -n app/src/main/java
grep -R "RapidInputViewModel(" -n app/src/test app/src/androidTest
grep -R "showDuplicateWarning" -n app/src/main/java app/src/test app/src/androidTest
```

## 5. Urutan Eksekusi

### 5.1 Static Test Inventory

```bash
find app/src/test app/src/androidTest -type f | sort
grep -R "@Test" -n app/src/test app/src/androidTest
```

Bandingkan test yang ada dengan daftar test terbaru pada bagian 9. Jika test belum memiliki file/method, beri status `Need Implementation`.
...
### 5.3 Instrumented Test Android

Jalankan hanya jika emulator atau device tersedia.
...
Jika tidak ada device, status seluruh instrumented test adalah `Blocked - No Android emulator/device`.

## 6. Standar Status Hasil

Gunakan status berikut secara konsisten:

| Status | Makna |
|---|---|
| Pass | Test berjalan dan semua assertion sesuai. |
| Failed | Test berjalan, tetapi assertion gagal atau aplikasi crash. |
| Blocked | Test tidak bisa dijalankan karena environment, dependency, compile error, emulator tidak tersedia, atau secret/staging belum disiapkan. |
| Need Implementation | Test scenario sudah wajib pada baseline terbaru, tetapi file/method test belum dibuat. |
| Need Refactor | Test lama ada, tetapi tidak lagi sesuai constructor, state, atau struktur ERD terbaru. |

## 7. Bukti yang Harus Dikumpulkan

Simpan hasil pada folder:

```text
testing-evidence/whitebox-updated/
```
...
gradle-android-test.log
static-test-inventory.txt
failed-tests-summary.md
screenshots-or-html-report/
```

Laporan Gradle default:
...
1. Jangan memakai database produksi.
2. Jangan menyimpan secret Supabase/Groq asli ke log.
3. Jangan memberi status `Pass` pada test kandidat yang belum dibuat.
4. Jangan mengubah source code utama kecuali task secara eksplisit meminta fixing.
5. Jangan menghapus staging data atau remote data nyata.

## 9. Daftar Test Case Terbaru
...
## 10. Format Ringkasan Akhir Agent

Agent harus mengembalikan ringkasan seperti berikut:
